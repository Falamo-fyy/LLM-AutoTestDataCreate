...
一款浏览器扩展（Manifest V3），自动识别网页表单字段并填充测试数据。支持中英文数据、AI智能推断、关联字段一致性生成、数据集保存与导入导出。
...
### 数据集管理
- **保存数据集**：填充数据后可保存为命名数据集
- **加载数据集**：打开 popup 即可选择已有数据集快速填充
- **URL 智能匹配**：优先显示与当前页面匹配的数据集
...
4. **保存数据集**：填充后可保存当前数据，便于后续复用
5. **加载数据集**：从下拉框选择已有数据集快速填充
6. **编辑修改**：填充后可编辑字段类型，重新生成单个字段
...
autodata-extension/
├── background/
│   └── service-worker.js    # AI 调用 + 配置管理 + 数据集存储
├── content/
